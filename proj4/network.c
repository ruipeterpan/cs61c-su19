...
// Optimized chunks 3403887μs
// OpenMP on the outer the for loop
void net_classify(network_t* net, volume_t** input, double** likelihoods, int n) {
  #pragma omp parallel
  {
    ...
    int end = (num_threads - 1 == thread_num) ? n : (start + n / num_threads);
