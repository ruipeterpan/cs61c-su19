...
'''
flatMap() outputs a list of tuples, in this case [('Amendment', 'Document1'), ('The', 'Document1')]
After distinct(), there are no duplicate items, so mapFunc gets rid of the document ID
The else is just the same as wordCount.py
Amendment: 10
the: 9
arms: 1
'''
def flatMapFunc(document):
    ...
    words_list = []
    for word in words:
        words_list.append((word, documentID))
    return words_list
