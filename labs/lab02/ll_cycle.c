...
int ll_has_cycle(node* head) {
  ...
  while (1) {
    if (hare == NULL || hare->next == NULL || hare->next->next == NULL) return 0;
    hare = hare->next->next;
    tortoise = tortoise->next; // A null pointer check is unnecessary because we already checked that two lines above.
