...
main:	jal	create_default_list 
	...
	#issue the map call 
	add	a0, s0, x0	# load the address of the first node into a0 Original Code
...
mapLoop: 
	...
	add t2, t2, t2       # new code
    add t2, t2, t2       # new code
	add	t1, t1, t0		# offset the array address by the count 
	lw	a0, 0(t1)		# load the value at that address into a0 
	
    addi sp, sp, -12   # fresh new code
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
	...
	#addi	t0, t0, 1		# increment the count Original Code
    addi	t0, t0, 4		# increment the count 

	bne	t0, t2, mapLoop	# repeat if we haven't reached the array size yet 
	 
	#la	a0, 8(s0)		# load the address of the next node into a0 Original Code
